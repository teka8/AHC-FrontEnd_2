import { Partner } from './partners'; // Assuming Partner type is defined in partners.ts

export const localPartners: Partner[] = [
  {
    name: "Wollo University",
    contact: {
      website: "https://wu.edu.et/",
      contactPerson: { name: "Contact Person", description: "University Representative", email: "info@wollo.edu.et" },
    },
    galery: ["/images/partners/wollo/wollo-galery1.jpg", "/images/partners/wollo/wollo-galery2.jpg", "/images/partners/wollo/wollo-galery3.jpg", "/images/partners/wollo/wollo-galery4.jpg"],
    website: "https://wu.edu.et/",
    logo: "/images/partners/wollo/wollo-university.png",
    description: {
      descriptionTitle: "",
      description: `<p>Wollo University (WU) is one of Ethiopia’s applied science public universities, established in 2005 as part of the country’s second-generation universities. It is located in Dessie City in the Amhara National Regional State, South Wollo Zone, about 400 km north of Addis Ababa. The university operates across three campuses: the Dessie Main Campus, the Kombolcha Institute of Technology (KIoT), and the Tita Tertiary Care Campus (TCC). It also hosts three centers of excellence dedicated to health sciences, technology, and art and culture.</p>
      <p>WU began with the Faculty of Education and quickly expanded to include the Faculties of Business and Economics, Informatics, and Engineering. By the 2009/10 academic year, these units were reorganized into colleges, institutes, and schools. Currently, the university consists of academic units distributed across its three campuses.</p>
      <p>The Tertiary Care Campus functions as the College of Medicine and Health Sciences. It encompasses Medicine, Nursing and Midwifery, Medical Laboratory Sciences, Pharmacy, Public Health, and the School of Environmental Health.</p>
      <p>Currently, Wollo University serves more than 20,000 students and offers over 83 undergraduate and 134 postgraduate programs, including 7 PhD and 5 specialty programs. These programs are delivered through regular, extension, summer, and distance education modalities. In addition to teaching, the university actively engages in multidisciplinary research and community service, guided by thematic areas such as health, agriculture, science and technology, education, business and economics, culture, art, and indigenous knowledge. These efforts contribute to both community development and the enhancement of the teaching-learning process.</p>`
    },
    scholars: [],
  },
  {
    name: "Wolkite University",
    contact: {
      website: "https://wku.edu.et/",
      contactPerson: { name: "Contact Person", description: "University Representative", email: "info@wku.edu.et" },
    },
    galery: ["/images/partners/wolkite/wolkite-galery1.jpg", "/images/partners/wolkite/wolkite-galery2.jpg", "/images/partners/wolkite/wolkite-galery3.jpg", "/images/partners/wolkite/wolkite-galery4.jpg"],
    website: "https://wku.edu.et/",
    logo: "/images/partners/wolkite/wolkite.jpg",
    description: {
      descriptionTitle: "",
      description: `<h2 style="margin: 0;"> 1.	Introduction </h2> <p>
Wolkite University is a Federal Higher Education Institution established by proclamation No.650/2009 and Establishment Regulation of the Council of Ministers No219/2011 under the Federal Ministry of Education (MOE), and responsible to teach students in undergraduate and graduate programs, conduct impactful researches, and provides wide ranges of community services.
<b>Vision</b>: To be one of the top ten universities in East Africa by 2030. 
<b>Mission</b>: Includes producing graduates who are knowledgeable, attitudinally mature, and practically innovative; supplying relevant and demanded technology and knowledge to address national and community development problems; and providing training and consulting services to the community and government.
<b>Core Values</b>: Excellence; inclusiveness; truth/integrity; being a learning organization; accountability; academic freedom.
</p>
   
<h2 style="margin: 0;"> 2.	Centers of Excellence and Collaboration Areas </h2> 
<p>
For Wolkite University, creating partnerships and fostering research collaborations in specialized fields such as health, engineering, agriculture business and economics and Indigenous knowledge through international relations can significantly elevate the institution’s academic and research profile.  
</p>  
<h2 style="margin: 0;"> 3.	Bilateral and Multilateral Research Partnership </h2> 
<p> 
Description: Wolkite University can engage in bilateral or multilateral partnerships with local or foreign universities, research institutions, and development organizations to jointly conduct research in the key sectors of health, engineering, agriculture, business and economics and indigenous knowledge. 
Example: Collaborating with broad universities in research related to sustainable technologies, healthcare delivery systems or agribusiness innovations.
Outcomes: Joint research publications, funding from international agencies, development of new technologies or practices, and enhanced research visibility. 
</p>
<h2 style="margin: 0;"> 4.	Overview of Partnership  </h2> 
<p>
Wolkite University is a public higher learning institution validly existing under the laws of Ethiopia. The University has the corporate power and authority to enter into and perform its obligations under this partnership contract, to act as an independent contractor to perform the Services. The institution will contribute to the AHEHC’s strategic objectives across the areas of health employment, health entrepreneurship,
 health ecosystem, and network development and learning. 
 </p>
`


    },
    scholars: [],
  },
  {
    name: "Debre Berhan University",
    contact: {
      website: "https://dbu.edu.et/",
      contactPerson: { name: "Contact Person", description: "University Representative", email: "info@dbu.edu.et" },
    },
    galery: ["/images/partners/debrebrehan/db-galery1.jpg", "/images/partners/debrebrehan/db-galery2.jpg", "/images/partners/debrebrehan/db-galery3.webp"],
    website: "https://dbu.edu.et/",
    logo: "/images/partners/debrebrehan/Debrebrehan.jpg",
    description: {
      descriptionTitle: "",
      description: `<p>Debre Berhan University (DBU), established in 2007 in the historic town of Debre Berhan 130 km from Addis Ababa. The university began with 725 students, one faculty, 68 instructors, and 7 administrative staff. Currently, it serves more than 20,000 students across regular, extension, summer, and distance programs.</p>
      <p>DBU currently offers 54 undergraduate, 64 postgraduate, and 5 PhD programs under nine colleges. It also implements Ethiopia’s new education roadmap launched in 2019.</p>
      <p>The university is expanding through major projects, including the Professor Asrat Woldeyes Health Science Campus, Hakim Gizaw Memorial Hospital, and ongoing developments such as the engineering college expansion and the Mehal-Meda Highland Agriculture and Tourism Research Center.</p>
      <p>With over 1,368 academic staff and 1,630 administrative staff, DBU aims to produce competent graduates, conduct problem-solving research, and strengthen community engagement. Its research focuses on indigenous knowledge, highland agriculture, herbal medicine, and technology, aligning with Debre Berhan’s growing industrial environment.</p>
      <p>DBU continues to build strong partnerships with local communities, government bodies, and NGOs to achieve its 2030 vision.</p>`
    },
    scholars: [],
  },
  {
    name: "Woldia University",
    contact: {
      website: "https://wldu.edu.et/",
      contactPerson: { name: "Contact Person", description: "University Representative", email: "info@wu.edu.et" },
    },
    galery: ["/images/partners/woldia/Woldia-galery1.webp", "/images/partners/woldia/Woldia-galery2.jpg", "/images/partners/woldia/woldia-galery3.jpg", "/images/partners/woldia/woldia-galery4.jpg"],
    website: "https://wldu.edu.et/",
    logo: "/images/partners/woldia/woldia.png",
    description: {
      descriptionTitle: "Woldia University is a public university in Woldia, Amhara Region, Ethiopia.",
      description: `Woldia University is one of the public universities established by the Government of Ethiopia following its plan to expand higher education institutions in the country’ to increase access and shore up its economic development and social transformation. Education coverage of Ethiopia so minimal even compared to the Sub-Saharan Africa. The Government of Ethiopia exerted its effort in expansion of education in the country to increase its educated human capital that meets the labor market demand of the nation. With this intention, the University was established in 2011 in Woldia town, North Wollo Zone, Amhara National Regional State on a 201-hectare. Currently the university has three campuses: Woldia Campus (main), Mersa Agricultural College, and St. Lalibela Institute of Heritage and Tourism Studies. Woldia is located in a cross-sectional geographic location which interconnects Samara (Afar), Mekele (Tigray) and Bahir Dar (Amhara), Dessie (Amhara) and in a distance of 520 KMs north of the Ethiopian capital, Addis Ababa.

  When the University began its teaching-learning process in 2011, it enrolled 573 regular undergraduate students in four faculties of twelve departments, and 156 week end (extension) undergraduate students in two departments, having 115 academic and 117 administrative staffs. Currently, the university has two institutes, two colleges, four faculties and six schools. Extensive work is being done in the community service sectors. The university has 1,285 teachers and technical assistants, 958 permanent administrative staffs, and 983 members of service providing Associations to whom campus cleaning, security, catering and cafeteria, and many other services are outsourced.

The Federal democratic republic of Ethiopia, ministry of Education tried to differentiate higher education institutions to play a great role on the national development agenda. Higher education institution in Ethiopia has been differentiated based on their generations, facilities, geographic location and the natural resource they have in their mandate area. The differentiation reform planned for the higher education sector has been focused on thematic areas including access, equity, and unity with diversity, quality, relevance, efficiency, research, technology transfer and community services, and the financing of higher education.

Since its establishment by the Government of Ethiopia, in 2011 G.C. the university had passed through a noticeable development progress for the past ten (12) years. Laboratories, workshops, libraries, offices, student services, ICT infrastructures, staff residence, guesthouses, electrical and water supply infrastructures, teacher’s residence, etc. were all up to par.

The university has competitive advantages in the area of tourism, agriculture, technology and health.  So in our 5 years strategic plan we identified tourism, agriculture, technology and health as a major area of excellence. Thus the departure mainly started from the listed center of Excellences of the university. 

If you would like to study in the university in the heart of the city that focus on changing the world for better to morrow, you’re choosing the right place. We do not use special formulas to select students. We look at every single applicant’s application, academic and personal, to select students who suit to our community with a full range of backgrounds. If you would like to study`
    },
    scholars: [],
  },
];