...
const ProgramDetail: React.FC = () => {
  ...
  const heroSummary = React.useMemo(() => {
    const shortDescription = typeof (program as any)?.short_description === 'string'
    ...
    () => [
      {
        ...
        description: countryChips.length
          ? `Active across ${countryChips.length} ${countryChips.length === 1 ? 'country' : 'countries'}${countryChips.length <= 3 ? ` (${countryChips.join(', ')})` : ''}.`
      ...
      },
      {
        ...
        description: partnerCount
          ? `Powered by ${partnerCount} strategic partner${partnerCount === 1 ? '' : 's'} driving shared outcomes.`
      ...
      },
      {
        ...
        description: contactCount
          ? `${contactCount} dedicated contact${contactCount === 1 ? '' : 's'} ready to assist applicants and stakeholders.`
    ...
    <div className="bg-white text-gray-800">
      <section className="relative isolate overflow-hidden">
        <div className="absolute inset-0">
          <img src={imageSrc} alt={program.title} className="h-full w-full object-cover" />
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-10 py-24 lg:py-32">
          <nav className="flex items-center gap-2 text-sm font-semibold text-white/70 mb-10">
            {breadcrumbs.map((crumb, index) => (
              <React.Fragment key={crumb.label}>
                ...
                {index !== breadcrumbs.length - 1 && <span className="text-white/50">•</span>}
              </React.Fragment>
            ))}
          </nav>
          ...
          <div className="max-w-4xl space-y-8 text-white">
            <div className="inline-flex items-center gap-3">
              <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wide ${statusStyle.bg} ${statusStyle.text}`}>
                {statusLabel}
              </span>
              <span className="text-xs uppercase tracking-[0.2em] text-white/70">Program Spotlight</span>
            </div>
            ...
            <p className="max-w-3xl text-base md:text-lg text-white/80">
              {heroSummary || 'Discover how this initiative is transforming health outcomes across the continent through collaborative innovation and bold partnerships.'}
            </p>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-stretch">
              <div className="backdrop-blur-md bg-white/10 border border-white/20 rounded-2xl px-6 py-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs uppercase tracking-wide text-white/70">Host Organisation</p>
                  <p className="font-semibold text-white text-lg">{program.host}</p>
                </div>
              </div>
          ...
          <div className="grid lg:grid-cols-12 gap-12">
            <div className="lg:col-span-8 space-y-12">
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="rounded-2xl border border-white/70 bg-white shadow-lg px-6 py-5 backdrop-blur">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Host organisation</p>
                  <p className="mt-3 text-lg font-semibold text-gray-900">{program.host}</p>
                  <p className="text-xs text-gray-400">Leading the delivery of this initiative.</p>
                </div>
                <div className="rounded-2xl border border-white/70 bg-white shadow-lg px-6 py-5 backdrop-blur space-y-3">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Program status</p>
                  <span className={`inline-flex w-max items-center px-3 py-1 text-xs font-semibold rounded-full ${statusStyle.bg} ${statusStyle.text}`}>
                    {statusLabel}
                  </span>
                  <p className="text-xs text-gray-400">{statusNarrative}</p>
                </div>
                <div className="rounded-2xl border border-white/70 bg-white shadow-lg px-6 py-5 backdrop-blur space-y-3">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Countries</p>
                  {countryChips.length ? (
                    <div className="flex flex-wrap gap-2">
                      {countryChips.map((country) => (
                        <CountryBadge key={`${program.id}-snapshot-country-${country}`} country={country} />
                      ))}
                    </div>
                  ) : (
                  ...
                  <p className="text-xs text-gray-400">
                    {countryChips.length ? `Expanded across ${countryChips.length} location${countryChips.length === 1 ? '' : 's'}.` : 'Designed for multi-region collaboration.'}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/70 bg-white shadow-lg px-6 py-5 backdrop-blur space-y-3">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Partner network</p>
                  <p className="text-lg font-semibold text-gray-900">{partnerCount ? `${partnerCount} partner${partnerCount === 1 ? '' : 's'}` : 'Announcing soon'}</p>
                  <p className="text-xs text-gray-400">
                    {partnerCount ? 'Jointly shaping curriculum, research, and delivery.' : 'Institutional collaborators are being finalised.'}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/70 bg-white shadow-lg px-6 py-5 backdrop-blur space-y-3">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Dedicated contacts</p>
                  <p className="text-lg font-semibold text-gray-900">{contactCount ? `${contactCount} team member${contactCount === 1 ? '' : 's'}` : 'To be announced'}</p>
                  <p className="text-xs text-gray-400">
                    {contactCount ? 'Connect directly for programme guidance.' : 'Support contacts will be shared shortly.'}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/70 bg-white shadow-lg px-6 py-5 backdrop-blur space-y-3">
                  <p className="text-xs uppercase tracking-wide text-gray-500">Pillars</p>
                  {categories?.length ? (
                    <div className="flex flex-wrap gap-2">
                      {categories.map((label) => (
                        <span key={`${program.id}-snapshot-pillar-${label}`} className="inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full bg-ahc-green/10 text-ahc-green-dark">
                          {label}
                        </span>
                      ))}
                    </div>
                  ) : (
                    <p className="text-sm font-semibold text-gray-900">Cross-pillar collaboration</p>
                  )}
                  <p className="text-xs text-gray-400">Focus areas guiding programme outcomes.</p>
                </div>
              </div>
              ...
              <article className="relative overflow-hidden rounded-3xl border border-white/70 bg-white shadow-xl">
                ...
                <div className="relative p-10 space-y-6">
                  ...
                  <h2 className="text-3xl font-bold text-gray-900">A deeper look into the initiative</h2>
                  <div className="prose prose-lg max-w-none text-gray-700" dangerouslySetInnerHTML={{ __html: program.description || '' }} />
                </div>
              </article>
              ...
              <article className="relative overflow-hidden rounded-3xl border border-white/70 bg-white shadow-xl space-y-8 p-10">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="pl-2 border-l-4 border-ahc-green">
                    <p className="text-xs uppercase tracking-[0.3em] text-ahc-green-dark">Collaborators</p>
                  ...
                  <span className="text-sm font-semibold text-ahc-green-dark">
                    {partnerCount ? `${partnerCount} active partner${partnerCount === 1 ? '' : 's'}` : 'Partner list coming soon'}
                  </span>
                </div>
                    ...
                    {partnerInstitutions.map((partner, index) => (
                      ...
                      >
                        ...
                        <div className="relative flex items-start gap-4 p-6">
                          <span className="text-lg font-bold text-ahc-green/80">#{String(index + 1).padStart(2, '0')}</span>
              ...
              <section id="program-contacts" className="relative overflow-hidden rounded-3xl border border-white/70 bg-white shadow-xl p-10 space-y-8">
                            ...
                            <>
                              ...
                              <div className="p-6 space-y-4">
                                <div className="space-y-1">
                                  <p className="text-xs uppercase tracking-wide text-ahc-green-dark">Program contact</p>
            ...
            <aside className="lg:col-span-4 space-y-6">
              <div className="sticky top-8 space-y-6">
                <div className="rounded-3xl border border-white/70 bg-white shadow-xl backdrop-blur p-8 space-y-6">
                  <h3 className="text-xl font-bold text-gray-900">Program at a glance</h3>
                  <div className="space-y-4 text-sm text-gray-600">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-gray-500 mt-1">{statusNarrative}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="font-semibold text-gray-900">Host organisation</p>
                      <p>{program.host}</p>
                    </div>
                    <div className="space-y-1">
                      ...
                      {countryChips.length ? (
                        <div className="flex flex-wrap gap-2">
                          {countryChips.map((country) => (
                            <CountryBadge key={`${program.id}-aside-country-${country}`} country={country} />
                          ))}
                        </div>
                      ) : (
                    ...
                    <div className="space-y-1">
                      ...
                      <p>{partnerCount ? `${partnerCount} partner${partnerCount === 1 ? '' : 's'}` : 'To be announced'}</p>
                    </div>
                  </div>
                  <a
                ...
                {primaryContact && (
                  <div className="rounded-3xl border border-white/70 bg-gradient-to-br from-ahc-green/10 via-white to-ahc-blue/10 shadow-xl p-8 space-y-4 text-sm text-gray-700">
                    <h4 className="text-lg font-bold text-gray-900">Primary contact</h4>
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="text-sm font-semibold text-gray-900">{primaryContact.name || 'Not specified'}</p>
                        {primaryContact.contact && (
                          contactLink ? (
                            <a href={contactLink} className="inline-flex items-center gap-2 text-ahc-green font-semibold break-words">
                              <Mail className="w-4 h-4" />
                              {primaryContact.contact}
                            </a>
                          ) : (
                    ...
                    {primaryContact.bio && (
                      <p className="text-xs text-gray-600 whitespace-pre-line">{primaryContact.bio}</p>
                    )}
                  </div>
                )}
                ...
                <div className="rounded-3xl border border-white/60 bg-gradient-to-br from-ahc-green/10 via-white to-ahc-blue/10 shadow-xl p-8 space-y-4 text-sm text-gray-700">
                  <h4 className="text-lg font-bold text-gray-900">Looking to collaborate?</h4>
                  <p>Share your interest to co-design research, placements, or learning journeys aligned with this programme.</p>
                  <p className="text-xs text-gray-500">We’ll connect you with the relevant contact to explore next steps.</p>
                </div>
              </div>
            </aside>
          </div>
