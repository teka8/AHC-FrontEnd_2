...
export default function ResourceDetail() {
  ...
  const {
    ...
    isLoading: l1,
    isError: e1,
  } = useGetDocumentByIdQuery(numericId, { skip: !isDoc || !numericId }) as any;
  const {
    data: edu,
    isLoading: l2,
    isError: e2,
  } = useGetEducationalByIdQuery(numericId, {
    skip: !isEdu || !numericId,
  }) as any;
  const {
    data: oth,
    isLoading: l3,
    isError: e3,
  } = useGetOtherByIdQuery(numericId, { skip: !isOther || !numericId }) as any;
  ...
  const FileTypeBadge = ({ url, mime }: { url?: string; mime?: string }) => {
    ...
    return (
      <div className="w-full h-64 rounded bg-slate-100 border flex items-center justify-center text-base text-slate-600">
        {label}
      </div>
    );
  };
  ...
  return (
    <div className="container pt-24 pb-16 md:pb-24">
      <Helmet>
        <title>{title} – AHC</title>
        <meta name="author" content="Africa Health Collaborative" />
        <meta
          property="og:title"
          content={`${title} – Africa Health Collaborative`}
        />
        <meta
          property="og:description"
          content={`Explore the resource "${title}" from the Africa Health Collaborative (AHC) at Addis Ababa University. Access valuable health-related documents, educational materials, and other resources to support healthcare initiatives across Africa.`}
        />
        <meta property="og:type" content="website" />
        <meta
          property="og:url"
          content={`https://ahc.tewostechsolutions.com/resources/${section}/${id}`}
        />
        <meta
          property="og:image"
      ...
      {isLoading ? (
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md space-y-4">
                <div>
                  ...
                  <div className="font-medium text-lg">
                    {item.document_type || item.resource_type || "Resource"}
                  </div>
                </div>
                {item.author && (
                  <div>
                ...
                {item.subject_area && (
                  <div>
                    ...
                    <div className="font-medium">{item.subject_area}</div>
                  </div>
                )}
                {item.publication_date && (
                  <div>
                    ...
                    <div className="font-medium">{item.publication_date}</div>
                  </div>
                )}
                {item.published_at && (
                  <div>
                    ...
                    <div className="font-medium">{item.published_at}</div>
                  </div>
                )}
                <div className="pt-4 flex gap-3">
                  {fileUrl && isPreviewable(mimeType, fileUrl) && (
                    <a
                      href={fileUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="btn bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-800 dark:text-white font-semibold rounded-full px-6 py-3 transition-colors"
                    >
                      View
                    </a>
                  )}
                  {fileUrl && (
                    <a
                      href={fileUrl}
                      download
                      className="btn bg-ahc-green hover:bg-ahc-green-dark text-white font-semibold rounded-full px-6 py-3 transition-colors"
                    >
                      Download
                    </a>
                  )}
        ...
        <div className="text-center py-16">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2">
            Please check the URL or go back to the resource list.
          </p>
        </div>
      )}
    </div>
